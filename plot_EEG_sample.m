function plot_EEG_sample(data, sample_amount, axis, channels, channel_txt, varargin)
%multiplot_EEG Create figure and plot multiple signals, 
...
%   optional arguments: fig_name, fig_pos - figure parameters
    ...
    % choose randomly trials to visualize
    data = datasample(data, sample_amount, 'Replace', false);
    ...
    m = ceil(sqrt(sample_amount)); n = ceil(sample_amount/m);
    % plot all
    for i = 1:sample_amount
        ...
        if i+n <= sample_amount
            set(gca, 'XTickLabels',[]);
        end
    end
    suptitle(fig_name+" of "+sample_amount+" trials");
